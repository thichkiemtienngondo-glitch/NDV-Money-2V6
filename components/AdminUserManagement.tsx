...
import { User as UserType, LoanRecord, AppSettings } from '../types';
import { 
  Search, 
  User, 
  ChevronDown, 
...
const AdminUserManagement: React.FC<AdminUserManagementProps> = ({ users, loans, isGlobalProcessing, onAction, onLoanAction, onDeleteUser, onAutoCleanup, onFetchFullData, onRefresh, onBack, settings }) => {
                                       ...
                                       <span className="text-xs font-black text-blue-400">
                                         {(() => {
                                           if (loan.settlementType === 'PRINCIPAL') {
                                             return Math.round((loan.amount * (Number(settings.PRE_DISBURSEMENT_FEE) / 100)) + (loan.fine || 0)).toLocaleString();
                                           } else if (loan.settlementType === 'PARTIAL') {
                                             const pAmt = loan.partialAmount || 0;
                                             return Math.round(pAmt + ((loan.amount - pAmt) * (Number(settings.PRE_DISBURSEMENT_FEE) / 100)) + (loan.fine || 0)).toLocaleString();
                                           } else {
                                      ...
                                      <div className="space-y-1.5">
                                        ...
                                        <div className="pt-3 flex flex-col items-center gap-2 border-t border-white/5 mt-2">
                                          ...
                                          <div className="bg-white p-2 rounded-xl shadow-lg group/qr relative">
                                            <img 
                                              src={`https://img.vietqr.io/image/${u.bankBin || BANK_BINS[u.bankName?.toUpperCase() || ""] || "970436"}-${u.bankAccountNumber}-compact2.png?amount=${Math.round(loan.amount * (1 - settings.PRE_DISBURSEMENT_FEE / 100))}&addInfo=${encodeURIComponent(`CKNDV ${loan.id.replace(/-/g, '')}`)}&accountName=${encodeURIComponent(u.bankAccountHolder || "")}`} 
                                            ...
                                            <button 
                                              onClick={() => {
                                                const url = `https://img.vietqr.io/image/${u.bankBin || BANK_BINS[u.bankName?.toUpperCase() || ""] || "970436"}-${u.bankAccountNumber}-compact2.png?amount=${Math.round(loan.amount * (1 - settings.PRE_DISBURSEMENT_FEE / 100))}&addInfo=${encodeURIComponent(`CKNDV ${loan.id.replace(/-/g, '')}`)}&accountName=${encodeURIComponent(u.bankAccountHolder || "")}`;
                                          ...
                                          <div className="flex flex-col items-center">
                                            <p className="text-[6px] font-bold text-gray-500 uppercase">Số tiền giải ngân: {Math.round(loan.amount * (1 - settings.PRE_DISBURSEMENT_FEE / 100)).toLocaleString()} đ</p>
                                            <p className="text-[5px] font-black text-blue-400 uppercase tracking-tighter mt-0.5">Nội dung: CKNDV {loan.id.replace(/-/g, '')}</p>
                                          </div>
                                        </div>
                                      </div>
                                    ) : (
                                  ...
                                  {['CHỜ DUYỆT', 'ĐÃ DUYỆT'].includes(loan.status) && (
                                    <div className="flex flex-col items-center justify-center gap-1 py-2.5 bg-white/[0.03] border border-white/5 rounded-xl">
                                      ...
                                      <p className="text-xs font-black text-white tracking-tight">
                                        {(loan.amount * (1 - settings.PRE_DISBURSEMENT_FEE / 100)).toLocaleString()} VNĐ
                                      </p>
                                    </div>
                                  )}
                                </div>
